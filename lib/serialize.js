...
export function clockString(ms) {
  ...
  const msg = isNaN(ms) ? '--' : Math.floor(ms / 60000) % 60;
  const s = isNaN(ms) ? '--' : Math.floor(ms / 1000) % 60;
  return [h, msg, s].map((v) => v.toString().padStart(2, 0)).join(':');
}
...
export async function smsg(sock, msg, store) {
  ...
  sock.sendAlbumMessage = async (jid, medias, options = {}) => {
    ...
    const album = generateWAMessageFromContent(jid, { messageContextInfo: {}, albumMessage: { expectedImageCount: medias.filter(msg => msg.type === 'image').length, expectedVideoCount: medias.filter(msg => msg.type === 'video').length, ...(options.quoted ? { contextInfo: { remoteJid: options.quoted.key.remoteJid, fromMe: options.quoted.key.fromMe, stanzaId: options.quoted.key.id, participant: options.quoted.key.participant || options.quoted.key.remoteJid, quotedMessage: options.quoted.message } } : {}) } }, {});
